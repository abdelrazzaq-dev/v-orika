...
export function generateWeekdayNameInLocal(locale: string, options: { format: string }): string[] {
  ...
  for (let index = 0; index <= 6; index++) {
    weekdays.push(today.toLocaleDateString(locale, { weekday: options.format }))
    today.setDate(today.getDate() + 1)
  }
  return weekdays
}

export function convertToMS(values: { minutes?: number, hours?: number, days?: number }): number {
  ...
  var result = 0;
  if (values.minutes)
    result += values.minutes * minute
  if (values.hours)
    result += values.hours * hour
  if (values.days)
